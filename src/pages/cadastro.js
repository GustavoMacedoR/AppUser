...
export default class IconTextboxExample extends Component {
  ...
  salvar = async () => {
    ...
    if (this.state.NomeCorrida != "none" && this.state.Data != "none" && this.state.percurso != "none" && this.state.local != "none") {
      ...
      try {
        await axios.post('corrida/salvarTreinos', body)
  ...
  makedata = (itemID) => {
    ...
    if (itemID != 'NO-ID') {
      placeholder = moment(this.state.Data.substr(0, 10)).format('DD/MM/YYYY')
    }
  ...
  atualizar = async () => {
    ...
    if (this.state.NomeCorrida != "none" && this.state.Data != "none" && this.state.percurso != "none" && this.state.local != "none") {
      ...
      try {
        await axios.patch('corrida/update/' + eventoid.id, body)
        this.handleUploadPhoto()
        //this.props.navigation.state.params.retorno();
        //this.state.change = false;
        this.props.navigation.navigate('eventos');
    ...
    return (
      <Container>
        ...
        <Content>
          ...
          {itemId == 'NO-ID' && <Button block onPress={this.salvar} ><Text>salvar</Text></Button>}
          {itemId != 'NO-ID' && <Button block onPress={this.atualizar} ><Text>Atualizar</Text></Button>}
        </Content>
        ...
        <Footer>
          <FooterTab>
            ...
            <Button vertical style={{borderTopWidth:5,borderColor:'white'}}>
